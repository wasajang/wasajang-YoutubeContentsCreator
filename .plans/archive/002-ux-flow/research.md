...
### 🎯 통합 우선순위 → **plan.md v2에서 전면 재작성됨** (아래 §14 참조)

---

## 14. 프로덕트 비전 업데이트 (사용자 2차 피드백 반영)

> 2026-02-27 세션 중 사용자의 추가 비전 공유. 기존 plan.md Phase 0~E 구조 폐기, 완전 재작성 결정.

### 14.1 고객 여정 — 3가지 시작점

사용자가 프로젝트를 시작하는 경로가 3가지:

```
HomePage
├─ [A] 대본 작성부터 시작 → Script 입력 → 스타일 선택 → Storyboard → Generate → Animate
├─ [B] 영상 스타일부터 시작 → 프리셋 선택 → Script 입력 → Storyboard → Generate → Animate
└─ [C] Cast부터 시작 → 배우/장소/아이템 AI 생성 + 5장 선택 → Script 입력 → ...

어떤 경로든 선택한 옵션들은 전부 저장 → 이후 AI 프롬프트에 자동 반영
```

**핵심 원칙:** "어떤 것에서부터 시작하든 선택한 옵션 값이 저장되어 이후 프롬프트에 반영"

### 14.2 Cast 페이지 역할 재정의

- **이전 계획:** 단순 카드 라이브러리 관리 (추가/삭제/조회)
- **새 비전:** **AI 기반 에셋 생성 스튜디오** + 프로젝트 진입점
  - 프롬프트로 배우 얼굴, 촬영 장소, 소품 이미지를 AI 생성
  - 5개를 선택(덱 구성)해서 프로젝트로 투입
  - 프로젝트 워크플로우의 "시작점"이 될 수 있음

### 14.3 필요한 신규 페이지

| 페이지 | 라우트 | 용도 | MVP? |
|--------|--------|------|------|
| **CastPage** | `/cast` or `/my-cast` | AI 프롬프트로 배우/장소/아이템 생성 + 5장 선택 → 프로젝트 시작 | ✅ |
| **SettingsPage** | `/settings` | 크레딧 잔액, 구독 요금제, BYOK API 키 입력, 계정 설정 | ✅ |
| **AdminPage** | `/admin` | Dev 전용 — AI 프롬프트 설정, 프리셋 공개/비공개/soon 관리 | ✅ (내부용) |

### 14.4 AI 모델 선택 시스템

각 생성 단계(대본, 이미지, 영상, TTS)마다:
- **영상 스타일 프리셋이 기본 모델을 지정** (예: "Sci-Fi" → 이미지: Flux, 영상: Runway)
- **유저가 다른 모델로 변경 가능**
- 선택한 모델이 해당 단계의 AI 호출에 사용됨

### 14.5 API 키 모델 — 하이브리드 결정

| 모드 | 대상 | 작동 방식 |
|------|------|----------|
| **플랫폼 크레딧** (기본) | 일반 사용자 | 크레딧 구매 → 소비 → 플랫폼이 API 비용 지불 |
| **BYOK** (고급) | 파워유저/개발자 | 자기 API 키 입력 → 크레딧 무제한 → 자기가 API 비용 지불 |

Settings 페이지에서 모드 전환 가능.

### 14.6 미래 비전 (MVP 이후 — Phase 2 프로덕트)

- **UGC 마켓플레이스:** 유저가 프리셋(대본 프롬프트, 캐릭터 카드, 이미지/영상 프롬프트 세팅) 판매
- **수익 공유:** 다른 유저가 해당 프리셋으로 크레딧 소비 시 → 일부가 프리셋 제작자에게 적립
- **YouTube 플랫폼 모델:** 크리에이터가 영상 스타일을 공유/판매

→ 이 기능은 현재 plan.md 범위 밖. MVP 매출 검증 후 별도 계획.

### 14.7 Dev Admin 페이지

- 각 AI 단계별 프롬프트 템플릿 편집
- 프리셋(영상 스타일) 관리: 공개 / "곧 출시" / 비공개
- AI 모델 설정값 관리
- Dev만 접근 가능 (인증 + 권한 체크)

---

*이 업데이트된 리서치를 기반으로 plan.md v2가 작성됩니다.*
