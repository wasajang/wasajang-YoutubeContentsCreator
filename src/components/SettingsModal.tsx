/**
 * SettingsModal — 설정 페이지를 모달 오버레이로 표시
 *
 * 페이지 이동 없이 현재 작업 위에 설정을 열고 닫을 수 있습니다.
 * 기존 작업 상태가 유지됩니다.
 */
import React, { useState, useEffect } from 'react';
import { X, Settings, Key, CreditCard, User, Eye, EyeOff, Plus, Trash2, AlertTriangle } from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
    ...
    // X 버튼/배경 클릭 시: history.back()으로 pushState를 제거 → popstate → onClose
    const handleClose = () => {
        window.history.back();
    };
